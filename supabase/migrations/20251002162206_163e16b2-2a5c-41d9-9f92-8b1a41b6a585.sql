...
CREATE TABLE public.projects (
  ...
  project_type text NOT NULL, -- 'Feature Film', 'TV Series', 'Commercial', etc.
  description text,
  status text DEFAULT 'open', -- 'open', 'casting', 'closed'
...
CREATE TABLE public.project_roles (
  ...
  requirements jsonb, -- skills, physical traits, etc.
...
CREATE POLICY "Directors can create projects"
...
CREATE POLICY "Directors can update own projects"
ON public.projects FOR UPDATE
TO authenticated
USING (director_id = auth.uid());

CREATE POLICY "Directors can delete own projects"
ON public.projects FOR DELETE
TO authenticated
USING (director_id = auth.uid());

-- RLS Policies for project_roles
CREATE POLICY "Project roles viewable by everyone"
ON public.project_roles FOR SELECT
USING (true);

CREATE POLICY "Directors can manage roles for their projects"
ON public.project_roles FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM public.projects 
    WHERE id = project_id AND director_id = auth.uid()
  )
);
...
TO authenticated
WITH CHECK (
  talent_id = auth.uid() AND
  public.has_role(auth.uid(), 'talent')
);
...
CREATE POLICY "Directors can update applications for their projects"
ON public.applications FOR UPDATE
TO authenticated
USING (
  EXISTS (
